...
{
    private string bundleURL; //= Application.absoluteURL;//"https://jelly.su/RubikCube/AssetBundles/";
    private char[] urlmas;
    ...
    //private string currentURL = Application.absoluteURL;
    //sripts
    ...
    private void Start()
    {
        bundleURL = Application.absoluteURL;
        MainCube = GameObject.FindWithTag("cube");
        urlmas = bundleURL.ToCharArray();
        bundleURL = "";
        for (int i = 0; i < (urlmas.Length - 10); i++)
        {
            bundleURL += urlmas[i];
        }
        bundleURL += "AssetBundles/";
    ...
    private IEnumerator DownloadCacheView(string name)
    {
        ...
        if (counter == 10 && name == "111")
        {
            Application.OpenURL("https://jelly.su/ContactsForMirea/index.html");
        }
        if (name == "chillout1")
        {
            counter++;
    ...
    public void ShowDestinationPlane(byte DestinText)
    {
        ...
        //UI_DestinitionText.text = ShowDestinitionObject.SetNewText(DestinText);
    ...
    public void BackToCube()
    {
        ...
        //PanoramSceneObjects.SetActive(false); // mb turn on
